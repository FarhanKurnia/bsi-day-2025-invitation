    ...
    const validateGuest = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                // Tangani kasus file tidak ditemukan atau error loading
                throw new Error(`Gagal memuat file JSON: ${response.statusText}`);
            }
            const data = await response.json();
            registeredGuests = data.guests;
        } catch (error) {
            console.error("Gagal memuat file tamu (Pastikan registered_guests.json ada):", error);
    ...
    openButton.addEventListener('click', () => {
      if(openButton.disabled) return; // Pengamanan tambahan
